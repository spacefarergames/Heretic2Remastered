...
// gl3_Model.c
//
// OpenGL 3.3 model loading (brush, sprite, book, flex models).
// Based on gl1_Model.c -- Copyright 1998 Raven Software
//
...
static void Mod_LoadBookModel(model_t* mod, const void* buffer, const int length)
{
	const book_t* book_in = buffer;
	book_t* book_out = Hunk_Alloc(length);

	if (book_in->bheader.version != BOOK_VERSION)
		ri.Sys_Error(ERR_DROP, "Mod_LoadBookModel: '%s' has wrong version number (%i should be %i)!", mod->name, book_in->bheader.version, BOOK_VERSION);

	if (book_in->bheader.num_segments > MAX_FRAMES)
		ri.Sys_Error(ERR_DROP, "Mod_LoadBookModel: '%s' has too many frames (%i > %i)!", mod->name, book_in->bheader.num_segments, MAX_FRAMES);
...
static void Mod_LoadSpriteModel(model_t* mod, const void* buffer, const int length)
{
	...
	for (int i = 0; i < sprout->numframes; i++)
	{
		...
		Com_sprintf(sprite_name, sizeof(sprite_name), "Sprites/%s", sprout->frames->name);
...
#pragma region ========================== BRUSH MODEL LOADING ==========================
...
static void Mod_LoadFaces(model_t* loadmodel, const byte* mod_base, const lump_t* l)
{
	...
	for (int surfnum = 0; surfnum < count; surfnum++, in++, out++)
	{
		...
		if (out->texinfo->flags & SURF_WARP)
		{
			...
			if (out->texinfo->flags & SURF_UNDULATE)
		...
		if (!(out->texinfo->flags & SURF_FULLBRIGHT))
...
static void Mod_LoadLeafs(model_t* loadmodel, const byte* mod_base, const lump_t* l)
{
	...
	const float offset = ((int)gl_noartifacts->value ? 32.0f : 0.0f);

	for (int i = 0; i < count; i++, in++, out++)
	{
		for (int j = 0; j < 3; j++)
		{
			out->minmaxs[j + 0] = LittleShort(in->mins[j]) - offset;
			out->minmaxs[j + 3] = LittleShort(in->maxs[j]) + offset;
		}
...
struct model_s* RI_RegisterModel(const char* name)
{
	...
	switch (mod->type)
	{
		...
		case mod_sprite:
		{
			...
			for (int i = 0; i < sprout->numframes; i++)
			{
				Com_sprintf(img_name, sizeof(img_name), "Sprites/%s", sprout->frames[i].name);
